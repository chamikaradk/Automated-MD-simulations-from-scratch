...
def make_gro(location,code_name):
    ...
    periodic_file = open("{}_UC.gro".format(code_name),"w")
    molecule_file = open("TOP/{}.gro".format(code_name), "w")
    ...
    periodic_file.close()
    molecule_file.close()
    mols[0].pmgmol.to(fmt="xyz",filename="DFT/{}.xyz".format(code_name))

if __name__ == '__main__':  #run this way to pass input from bash main
    location = sys.argv[1]
    code_name = sys.argv[2]
    make_gro('CIF/'+location,code_name)


#make_gro('CIF/812876_NDT.cif','NDT')
